 ...
 * }} props
 */
export default function BookCard({ book, progress, isPinned, onOpen, onTogglePin, animClass }) {
      ...
      <div className="flex gap-4 pr-7">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex justify-between items-start mb-1.5">
            <span
              className="pill"
              style={{ color: genreColor, background: `${genreColor}18`, border: `1px solid ${genreColor}28` }}
            >
              {book.genre}
            </span>
          ...
          {progress !== undefined ? (
            <>
              <div className="flex justify-between mb-1.5">
                <span className="font-crimson text-[13px] text-ink-subtle">Ch. {progress} / {book.chapters}</span>
                <span className="font-crimson text-[13px] font-semibold" style={{ color: genreColor }}>{pct}%</span>
              </div>
              <div className="progress-track">
                <div
                  className="progress-fill"
                  style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${genreColor}70, ${genreColor})` }}
                />
              </div>
            </>
          ) : (
            <p className="font-crimson text-[13px] text-ink-ghost italic">Set your progress to join →</p>
          )}
        </div>
      </div>
    </div>
